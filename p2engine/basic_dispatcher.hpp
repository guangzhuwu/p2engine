...
public:\
	...
	typedef fssignal::signal<void(message_type,safe_buffer)> unknown_message_signal_type;\
	\
	...
	public:
		...
		message_signal_type& invalid_message_signal()
		{
			return invalid_message_signal_;
		}
		const message_signal_type& invalid_message_signal()const
		{
			return invalid_message_signal_;
		}
		unknown_message_signal_type& unknown_message_signal()
		{
			return unknown_message_signal_;
		}
		const unknown_message_signal_type& unknown_message_signal()const
		{
			return unknown_message_signal_;
		}
		...
		bool dispatch_packet(const safe_buffer& buf,const message_type& msg_type)
		{
			...
			if (!unknown_message_signal_.empty())
				unknown_message_signal_(msg_type,buf);
	...
	public:
		...
		message_signal_type invalid_message_signal_;
		unknown_message_signal_type unknown_message_signal_;
...
public:\
	...
	public:
		...
		received_signal_type& invalid_message_signal()
		{
			return invalid_message_signal_;
		}
		const received_signal_type& invalid_message_signal()const
		{
			return invalid_message_signal_;
		}
		unknown_message_signal_type& unknown_message_signal()
		...
		void dispatch_disconnected(const error_code& ec)
		{
			if (ec)
			{
				disconnected_signal_type signal;
				signal.swap(this->disconnected_signal());
		...
		void dispatch_connected(const error_code& ec)
		{
			if (ec)
			{
				connected_signal_type signal;
				signal.swap(this->connected_signal());
				disconnect_all_slots();
				signal(ec);
			}
			else
	...
	public:
		...
		bool dispatch_packet(const safe_buffer& buf)
		{
			//1. search <message_soceket,net_event_handler_type> bind in this socket
