...
//
// static std::size_t tcp_header::header_type::format_size();
// virtual std::size_t packet_size() const;
//
// that is, you can use the following code to get the declared packet size:
//
// std::size_t packet_sz = header.format_size();
// or
// std::size_t packet_sz = tcp_header::header_type::format_size();
// or
...
//
// Note that for bit field, the ``bytes_of_xxx" returns the bytes of shared_ptr defined in marcro:
// e.g. with
//    P2ENGINE_PACKET_BIT_FIELD_DEF    (uint8_t,    ACK,        1    )
// defined in Instruction 1,
//
...
// std::size_t tcp_header_extended::header_type::get_payload(p2net::safe_buffer &buf, std::size_t len);
// void tcp_header_extended::header_type::set_payload(const void * buf, std::size_t len);
// void tcp_header_extended::header_type::set_payload(const p2net::safe_buffer& buf, std::size_t len);
//
...
//
// std::size_t len = ext_header.truncated_size();//returned value is equal to ``ext_header.packet_size()"
//
...
//
// std::size_t new_len = ext_header.truncated_size();//returned value is equal to ``ext_header.packet_size() - 300"
//
...
protected: \
	...
	static const std::size_t SIG_BYTES = SIGNATURE_BITS?sizeof(signature_type):0; \
...
public: \
	...
	\
	virtual std::size_t packet_size() const {return PACKET_FORMAT_DEF::format_size();}; \
	\
...
public: \
	...
	{ \
	return sizeof(TYPE) * static_cast<std::size_t>(8); \
	};
...
public: \
	std::size_t get_##FIELD_NAME(void * buf, std::size_t buf_size, std::size_t len = (std::numeric_limits<std::size_t>::max)()) const \
	{ \
	std::size_t true_len = (std::min)(static_cast<std::size_t>(SIZE), len); \
	...
	if (ABS_HEAD_BYTE_OFFSET_OF_##FIELD_NAME + static_cast<std::size_t>(SIZE) == this->packet_size()) \
		{ \
		truncated_size_ = ABS_HEAD_BYTE_OFFSET_OF_##FIELD_NAME + len; \
		} \
	}; \
	std::size_t get_##FIELD_NAME(p2engine::safe_buffer& buf, std::size_t len = (std::numeric_limits<std::size_t>::max)()) const \
	{ \
	std::size_t true_len = (std::min)(static_cast<std::size_t>(SIZE), len); \
	...
	\
	void set_##FIELD_NAME(const p2engine::safe_buffer &buf, std::size_t len = (std::numeric_limits<std::size_t>::max)()) \
	{ \
	std::size_t true_len = (std::min)(buf.length(), len); \
	assert(true_len <= static_cast<std::size_t>(SIZE)); \
...
#    endif
	...
	public:
		TYPE operator()(const p2engine::safe_buffer &buf,std::ptrdiff_t offset_in_buf,
			std::size_t OFFSET_IN_TYPE, std::size_t BITS)
		{
	...
	public:
		void operator()(const p2engine::safe_buffer& buf,std::ptrdiff_t offset_in_buf, 
			std::size_t OFFSET_IN_TYPE, std::size_t BITS, TYPE val)
		{
			BOOST_ASSERT(offset_in_buf+sizeof(TYPE)<=buf.length());
			char* p=p2engine::buffer_cast<char*>(buf)+offset_in_buf;
			char *rp=p, *wp=p;
