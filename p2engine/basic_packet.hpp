...
namespace p2engine {
	...
	{
	public:
		basic_packet(std::size_t buffer_len) 
			: buffer_(buffer_len), packet_length_(buffer_len), user_specified_length_(false) 
		{};
		basic_packet(const safe_buffer& buf, std::size_t packet_len) 
	...
	public:
		...
		virtual std::size_t packet_length() const
		{
			return packet_length_;
		};

		virtual void packet_length(std::size_t length)
		{
		...
		std::size_t buffer_length() const {return buffer_.length();};
